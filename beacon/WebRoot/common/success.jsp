	...
	<head>
		...
		<script>
			jQuery(function() {
				...
				if (dg.topWin.actionSuccess) {
					dg.topWin.actionSuccess();
				} else if (dg.curWin.actionSuccess) {
					dg.curWin.actionSuccess();
				}
			});
		</script>
	</head>
	<body class="popup-page" style="overflow: hidden">
		<div>
			<div style="width: 50px; height: 150px; float: left">
				<div
					style="width: 50px; height: 50px;margin-top:30px;background: url(${ctx}/images/right.gif)"></div>
				...
				<div style="font-size: 14px; margin-left: 10px; padding-right: 10px;margin-top:35px">
					<%
						String message = (String) request
								.getAttribute(MRSConstants.MESSAGE_KEY);
						if (message == null) {
							message = "成功保存信息";
						}
