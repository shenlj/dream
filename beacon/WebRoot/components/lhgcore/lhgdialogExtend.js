...
$.fn.extend({
	...
	/* 
	 * function: 确认窗口
	 * parameter: 确认信息文本， 确认后执行的方法， 取消后执行的方法，窗口宽度， 窗口高度， 触发确认窗口的事件
	 *
	 */
 
	confirm: function(content, callback, cancel, width, height, event) {
		...
		$(this).live(event, function() {
			...
			var dg = frameElement ? frameElement.lhgDG : null;
			var confirmDG;
			...
			if (dg) {
				confirmDG = new dg.curWin.J.dialog({
					...
					width: width,
					height: height,
					html: '<div class="alert-question">' + content + '</div>',
					resize: false,
					cover: true,
					rang: true,
					btns: true,
			...
			confirmDG.addBtn('yes', '确定', function() { 
				confirmDG.cancel(); 
				obj.callback(); 
			});
			confirmDG.addBtn('no', '取消', function() {
				confirmDG.cancel();
				obj.cancel();
			});
...
jQuery.extend({
	...
	/* 
	 * function: 弹出窗口
	 * parameter: 窗口标题， 窗口内容页面url， 窗口宽度， 窗口高度
	 *
	 */
 	 
	dialog: function(title, url, width, height, otherParams) {	
		var dg = frameElement ? frameElement.lhgDG : null;
		var dialogDG;
	...
	/* 
	 * function: 确认窗口
	 * parameter: 确认信息文本，确认后执行的方法，取消后执行的方法，窗口宽度，窗口高度
	 *
	 */
	 
	confirm: function(content, callback, cancel, width, height) {
	...
	/* 
	 * function: 提示窗口
	 * parameter: 提示信息文本， 窗口宽度， 窗口高度， 确认后执行的方法, 提示类型
	 *
	 */	
	...
	alert: function(content, width, height, callback, type) {
		...
		alertDG.addBtn('yes', '确定', function() { 
			alertDG.cancel(); 
			callback();
		});
	...
	/* 错误提示窗 */
	error: function(content, width, height, callback) {
	...
	try{
		...
		for (var i = 0; i< calendar.guid ; i++){
			...
			if(calendar[i] != undefined){
				if(calendar[i].elem.innerText == '提示信息' || calendar[i].elem.innerText == '确认信息'){
					closeFlg = false;
...
$(function() {
	...
	if ($frame && $frame.attr('id') == 'lhgfrm') {
		...
		$("html").live("keydown", function(event){cancleKeyDown(event);});
		$("body").focus();
	}
