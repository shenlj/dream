/*
 * ! lhgcore JavaScript Library v1.4.2 Date: 2011-02-12 09:23:11 Copyright (c)
 * 2009 - 2011 By Li Hui Gang
 */
(function(v, B) {
	var e = function(a, b) {
		return new e.fn.init(a, b)
	}, l = v.document, ta = Array.prototype.push, ua = Array.prototype.slice, U = Object.prototype.toString, V = Object.prototype.hasOwnProperty, ba = false, P = [], M, va = /^#[\w-]+$/, wa = /^(\s|\u00A0)+|(\s|\u00A0)+$/g;
	e.fn = e.prototype = {
		init : function(a, b) {
			if (!a)
				return this;
		...
		html : function(a) {
			...
			else if (typeof a === "string" && !za.test(a))
		...
		removeClass : function(a) {
			for (var b = 0, c = this.length, d, g = (a || "").split(ea); b < c; b++) {
				d = this[b];
				if (a) {
					for (var h = (" " + d.className + " ").replace(Aa, " "), i = 0, p = g.length; i < p; i++)
						h = h.replace(" " + g[i] + " ", " ");
		...
		filter : function(a) {
			return this.setArray(Ba(this, function(b, c) {
						return a.call(b, c, b)
					}))
		},
		is : function(a) {
	...
	e.extend = e.fn.extend = function() {
		...
		for (var d in b) {
			c = b[d];
			if (a !== c)
				a[d] = e.isPlainObject(c) && !Q(c) ? arguments.callee(a[d]
								|| {}, c) : c
		}
		return a
	};
	e.extend({
		...
		globalEval : function(a) {
			if (a && /\S/.test(a)) {
				var b = l.getElementsByTagName("head")[0] || l.documentElement, c = l
	...
	var W = e.isFunction, Q = e.isArray, I = e.nodeName, Y = e.map, X = e.inArray, N = e.toArray, J = e.browser, Ba = e.grep, ya = e(l), fa = function() {
		if (!e.isReady) {
			try {
	...
	e.extend({
				...
				event : {
					add : function(a, b, c) {
						...
						if (b && !e.cache[d].events[b]) {
							...
							if (a.addEventListener)
								a.addEventListener(b, e.cache[d].listener,
										false);
							else
								a.attachEvent
										&& a.attachEvent("on" + b,
					...
					remove : function(a, b, c) {
						try {
							var d = e.cache ? e.cache[a.lhguid] : B, g, h
						} catch (i) {
							return
						}
						if (d) {
	...
	v.ActiveXObject && v.attachEvent("onunload", function() {
	...
	(function() {
		var a = !!l.getElementsByClassName, b = !!l.querySelectorAll, c = /^[\w:.][\w-]*$/, d = / *, */, g = /([^[:.#]+)?(?:#([^[:.#]+))?(?:\.([^[:.]+))?(?:\[([^!&^*|$[:=]+)([!$^*|&]?=)?([^:\]]+)?\])?(?:\:([^(]+)(?:\(([^)]+)\))?)?/, h = /(-?)(\d*)n((?:\+|-)?\d*)/, i = /(~|>|\+)/, p = /(\([^)]*)\+/, q = /(\[[^\]]+)~/, K = / +/, j = /\]\[/g, t = {
			"" : function(f, k) {
		...
		e.find = function(f, k) {
			...
			if (c.test(f) && k.nodeType) {
				...
				switch (f.charAt(0)) {
					case "." :
						...
						if (a)
							...
							s = " " + s + " ";
							for (var z = k.getElementsByTagName("*"), A = 0, D; D = z[A++];)
				...
				for (var ha = D.length - 1, Fa = !!ha, ia, ja, n, $, E, F, G, x, y, R, C, ka, r, O, T, o, L; u = D[ha--];) {
					ja = (ia = u.replace(j, "] % [").replace(p, "$1%").replace(
							q, "$1&").replace(i, " $1 ").split(K)).length;
					A = 0;
					$ = " ";
					for (z = k.nodeType ? [k] : N(k); n = ia[A++];)
						if (n !== "%" && n !== " " && n !== ">" && n !== "~"
								&& n !== "+" && z) {
							n = n.match(g);
							E = n[1] || "*";
	...
	var la = l.defaultView || {}, ma = /href|src|style/, ea = /\s+/, Aa = /[\n\t]/g, na = /float/i, oa = /-([a-z])/ig, Ga = /([A-Z])/g, pa = {
		"class" : "className",
		"for" : "htmlFor"
	}, Ha = {
		"float" : J.ie ? "styleFloat" : "cssFloat"
	}, qa = function(a, b) {
	...
	e.extend({
				...
				getCSS : function(a, b) {
					...
					if (b == "opacity")
						return J.ie
								? d.filter && d.filter.indexOf("opacity=") >= 0
										? parseFloat(d.filter
												.match(/opacity=([^)]*)/)[1])
												/ 100
										: 1
	...
	var xa = /^<(\w+)\s*\/?>(?:<\/\1>)?$/, Ia = /(<([\w:]+)[^>]*?)\/>/g, Ja = /<|&\w+;/, Ka = /<([\w:]+)/, za = /<script|<object|<embed|<option|<style/i, La = /^(?:area|br|col|embed|hr|img|input|link|meta|param)$/i, Ma = function(
			a, b, c) {
		...
		if (!d) {
			...
			a : {
				...
				for (var p = [], q = 0, K = a.length, j; q < K; q++) {
					...
					if (!j)
						break a;
					if (typeof j === "string" && !Ja.test(j))
						j = b.createTextNode(j);
					else if (typeof j === "string") {
						j = j.replace(Ia, Ma);
						var t = (Ka.exec(j) || ["", ""])[1].toLowerCase();
						t = H[t] || H.rdefault;
						var w = t[0], f = b.createElement("div");
						for (f.innerHTML = t[1] + j + t[2]; w--;)
							f = f.lastChild;
						J.ie
								&& /^\s+/.test(j)
	...
	var Na = /select|textarea/i, Oa = /text|hidden|password|search/i, Pa = /(\?|&)_=.*?(&|$)/, Qa = /%20/g;
	e.fn.extend({
				serialize : function() {
				...
				serializeArray : function() {
					return this.map(function() {
						...
						return this.name
								&& !this.disabled
								&& (this.checked || Na.test(this.nodeName) || Oa
										.test(this.type))
					}).map(function() {
								var a = e(this).val();
								return a == null ? null : Q(a) ? Y(a, function(
												b) {
											return {
												name : elem.name,
												value : b
	...
	e.extend({
		...
		ajax : function(a) {
			...
			if (a.cache === false && d == "GET") {
				var g = (new Date).getTime(), h = a.url.replace(Pa, "$1_=" + g
								+ "$2");
				a.url = h
						+ (h == a.url ? (a.url.match(/\?/) ? "&" : "?") + "_="
								+ g : "")
			}
			if (a.data && d == "GET") {
			...
			var K = false, j = a.xhr();
			a.username ? j.open(d, a.url, a.async, a.username, a.password) : j
					.open(d, a.url, a.async);
			try {
				a.data && j.setRequestHeader("Content-Type", a.contentType);
				j.setRequestHeader("X-Requested-With", "XMLHttpRequest");
				j.setRequestHeader("Accept", a.dataType
			...
			var w = function(m) {
				if (j.readyState == 0) {
					...
					b = m == "timeout" ? "timeout" : !e.httpSuccess(j)
							? "error"
							: "success";
					if (b == "success")
						try {
		...
		httpData : function(a, b) {
			...
			var d = (c = b == "xml" || !b && c && c.indexOf("xml") >= 0)
					? a.responseXML
					: a.responseText;
