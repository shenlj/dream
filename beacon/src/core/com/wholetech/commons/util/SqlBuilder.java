...
public class SqlBuilder {
	...
	public static String insert(final String sTable, final String strRowName[], final String[] strRowValue) {

		// 要插入列的个数
		final int iLength = strRowName.length;
	...
	public static String insert(final String sTable, final Object strRowName[], final Object[] strRowValue) {
		...
		for (int i = 0; i < iLength - 1; i++) {
			if (strRowValue[i] instanceof Long) {
				...
				sql = sql.append(SqlBuilder.convertDate(strRowValue[i]));
		...
		if (strRowValue[iLength - 1] instanceof Long) {
			...
			sql = sql.append(SqlBuilder.convertDate(strRowValue[iLength - 1]));
	 ...
	 * <p>
	 * 例如： select * form user where 1=1 /~and sex=?~/ /~and name=?~/ 那么对于条件["m", IGNORE_FILTER]，则sql被解析成select * form
	 * user where 1=1 and sex=?
	 *
	 * @param srcSQL
	 *            原始SQL，带有/~ ~/语句块。
	 * @param filters
	 ...
	 * @return
	 */
	public static String parseSql(final String srcSQL, final Object[] filters) {

		// 转成StringBuffer，提高效率
		final StringBuffer bfSQL = new StringBuffer(srcSQL);

		for (int i = 0, start = 0, end = 0; (start = bfSQL.indexOf(SqlBuilder.SQL_RULE_OPEN, end)) >= 0; i++) {
			// 从上次结束到当前开始位置，可能有?存在，则把这样的参数给迈过去。
			i += StringUtils.countMatches(bfSQL.substring(end, start), "?");

			end = bfSQL.indexOf(SqlBuilder.SQL_RULE_CLOSE, start);
			if (end == -1) {
				throw new java.lang.IllegalArgumentException("srcSQL doesn't obey the sql format rule");
			}

			// 使用==就可以判断
			if (SqlBuilder.IGNORE_FILTER == filters[i]) {
				// 如果是忽略的过滤条件，则直接移除
				bfSQL.replace(start, end + SqlBuilder.SQL_RULE_CLOSE.length(), "");
				end = start;
			} else {
				final String real = bfSQL.substring(start + SqlBuilder.SQL_RULE_OPEN.length(), end);
				bfSQL.replace(start, end + SqlBuilder.SQL_RULE_CLOSE.length(), real);
	 ...
	 * @return 处理过的参数
	 */
	public static Object[] ignoreNullBlank(final Object[] filters) {

		final Object[] arr = new Object[filters.length];
		for (int i = 0; i < filters.length; i++) {
			if (filters[i] == null) {
				arr[i] = SqlBuilder.IGNORE_FILTER;
	 ...
	 */
	@Deprecated
	public static void ignoreBlankString(final Object[] filters) {

		for (int i = 0; i < filters.length; i++) {
			if (filters[i] instanceof String && StringUtils.isBlank((String) filters[i])) {
				filters[i] = SqlBuilder.IGNORE_FILTER;
			}
		}
	}

	private static String convertDate(final Object date) {
		...
		if ("derby".equals(Constants.CURRENT_DATABASE_TYPE)) {
			return sql.append(" TIMESTAMP('").append(DateUtil.format((Date) date, DateUtil.getDateTimePattern()))
					.append("'),").toString();
			...
			return sql.append(" convert(date,").append(DateUtil.format((Date) date, DateUtil.getDateTimePattern()))
					.append(",120),").toString();
		} else {
			return sql.append(" '").append(DateUtil.format((Date) date, DateUtil.getDateTimePattern())).append("',")
					.toString();
		}
	}
}
