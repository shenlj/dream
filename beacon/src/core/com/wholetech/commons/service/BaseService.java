...
/**
 * Service层对外公共接口。
 * 这一层主要封装业务逻辑，其中的重点是负责管理事务。从编码理论上讲，这一层的代码量应该是最大的，因为负责业务逻辑，所以一个Service可能引用几个Dao来完成复杂的业务逻辑。 <br />
 * Service编码要注意的点是： <li>事务的配置是在spring的ApplicationContext.xml文件中，参见元素&lt;tx:advice&gt;， 针对事务主要有两类，一类是要求有事务，一类不关心有没有事务。</li>
 * <li>当一个Service嵌套调用同一个Service中的方法，嵌套调用是完全不受事务控制的； 只有从其他类中调用service方法才受事务控制。</li> <li>
 * 事务的回滚完全是由异常来控制的，默认情况下，一旦抛出RuntimeException异常，则事务就会回滚。 对于checked异常，则需要配置在&lt;tx:method name="update*"
 * rollback-for="MyException"/&gt;</li>
 * <p>
 * 对接口中出现的术语进行解释：
...
public interface BaseService<T> {
	 ...
	 * 批量更新一批对象。
	 * <p>
	 * 更新后，这些对象的状态不变，原来如果不是持久化状态，现在依然不是持久化状态。
	 *
	 * @param detachedInstances
	 *            待更新状态。
	 */
	 ...
	 * 如果通过name=value没有找到任何对象，则唯一性成立； 如果只找到一个，主键值等于id，则唯一性成立，否则不成立； 如果找到多个，则不成立。
	 * <p>
	 * 如果id为空，则根据以下规则判断：<br>
	 ...
	 * 通过sql语句查询实体。
	 * <p>
	 * 如果select多个字段，则返回的对象是这些字段值组成的数组对象； 如果select一个字段，则返回这一个字段对应的对象。
	 *
	 * @param sqlOrKey
